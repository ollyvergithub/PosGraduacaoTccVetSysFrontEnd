...
export const Lista = ({
    ...
    const editarTemplate = (rowData) => {
        return (
            <Link
                to={{
                    pathname: `/edicao-de-veterinario/${rowData['uuid']}`,
                }}
        ...
        <>
            {veterinarios && veterinarios.length > 0 ?
                <DataTable
                    ...
                    showGridlines
                    paginator={veterinarios.length > rowsPerPage}
                    rows={rowsPerPage}
            ...
            <section>
                <ModalConfirmaExclusao
                    ...
                    texto={`<p>Deseja realmente excluir o veterinário.</p>`}
